...
> **بنية المجلدات:** المشروع كامل داخل `movies-scraper/`. مجلد `release/`
> داخله هو النسخة النظيفة المعدّة للرفع (بلا اختبارات وبلا أسرار). للرفع
> انسخ محتويات `release/` إلى الخادم، أو ارفع `movies-scraper/` كاملاً
> (الملفات الزائدة لا تضر لأن Dockerfile يعتمد على `film_scraper/` فقط).
...
cd Movie-Serial-Scraper/movies-scraper     # المشروع معزول داخل هذا المجلد
cp .env.example .env
```
