...
المشروع معزول في مجلده الخاص `movies-scraper/` لئلا يتعارض مع المشاريع الأخرى.
مصدر الكود الوحيد هو `film_scraper/` (قابلة للنشر مستقلة)، و`release/` هو مجلد
النتائج النهائية النظيفة المعدّة للرفع:

```
movies-scraper/              ← مجلد المشروع الكامل (تطوير)
├── film_scraper/            ← مصدر الكود الوحيد (single source of truth)
...
├── tests/                   # الاختبارات (تطوير فقط — لا تدخل release/)
├── android/                 # تطبيق أندرويد (تطوير فقط)
├── release/                 # مجلد النتائج النهائية: ملفات الإنتاج النظيفة للرفع
│   └── film_scraper/        #   نسخة إنتاج (بلا اختبارات، بلا .env، بلا كاش)
├── sync_release.py          # مزامنة الملفات المعدّلة فقط من dev إلى release/
├── Dockerfile               # بناء الحاوية الكاملة (متصفح) — سياق البناء movies-scraper/
├── Dockerfile.lite          # بناء الحاوية الخفيفة (HTTP نقي) — للخطط المجانية
├── docker-compose.yml       # التشغيل على VPS
├── render.yaml              # نشر Render
└── DEPLOYMENT.md            # دليل النشر
```

### سير العمل
1. تعدّل في `film_scraper/` و`tests/` (التطوير).
2. تختبر: `python -m pytest tests -q`.
3. تزامن فقط الملفات المعدّلة إلى `release/`:
   ```bash
   python sync_release.py              # ينسخ الملفات المعدّلة فقط
   python sync_release.py --check      # يعرض ما سيتزامن دون نسخ
   python sync_release.py --full       # إعادة بناء release/ بالكامل
   ```
4. `release/` هو ما ترفعه للخادم (نظيف بلا اختبارات وبلا أسرار).
...
للاختبارات (من جذر المشروع `movies-scraper/`):
```bash
